...
m = MorphAnalyzer()
query_text = "контекстная реклама"
...
sys.stdout = open("C:\\Users\\user\\Documents\\url_list.txt", "w", encoding="utf-8")
...
sys.stdout.close()
sys.stdout = open("C:\\Users\\user\\Documents\\out.txt", "w", encoding="utf-8")

# Нам лишь нужно найти наиболее часто употребляемые N-граммы в текстах ТОП 50.


# Приступаем к анализу полученных данных.
urls = [z.rstrip() for z in open('C:\\Users\\user\\Documents\\url_list.txt')]
for url in urls:
    try:
...
sys.stdout.close()
sys.stdout = open("C:\\Users\\user\\Documents\\result.txt", "w", encoding="utf-8")
texts = [z.rstrip() for z in open('C:\\Users\\user\\Documents\\out.txt', encoding='utf-8')]
stop_words = [z.rstrip() for z in open('C:\\Users\\user\\Documents\\stop_words.txt', encoding='utf-8')]
...
info_data = pd.read_csv('C:\\Users\\user\\Documents\\result.txt', encoding='utf-8', sep="\t", header=None)
filename = 'C:\\Users\\user\\Documents\\' + query_text + '.xlsx'
